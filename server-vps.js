...
/* ── LISTAR BACKUPS DISPONÍVEIS NO VPS ──
   GET /admin/listar-backups?senha=XXXX
   Retorna lista dos backups disponíveis, ordenados do mais recente.
*/
expressApp.get('/admin/listar-backups', (req, res) => {
  try {
    ...
    res.json({ ok: true, backups: arquivos });
  } catch(e) {
    res.status(500).json({ ok: false, erro: e.message });
  }
});

/* ── BAIXAR/LER BACKUP ESPECÍFICO ──
   GET /admin/ler-backup?senha=XXXX&nome=dados_XXX.json
   Retorna o conteúdo do backup escolhido.
*/
expressApp.get('/admin/ler-backup', (req, res) => {
  try {
    const senha = req.query.senha || '';
    ...
    // Validação básica de path traversal
    if (!nome.match(/^dados_[\w-]+\.json$/)) {
      return res.status(400).json({ ok: false, erro: 'Nome inválido' });
    }

    const caminho = path.join(backupDir, nome);
    ...
    // Conta faturas e finalizadas pra mostrar resumo
    let qtdFaturas = 0;
    if (Array.isArray(dados.dados)) {
      qtdFaturas = dados.dados.filter(p =>
        Array.isArray(p) && p.some(c => c && c !== '')
      ).length;
    }
    const qtdFinalizadas = (dados.finalizadas || []).filter(f => f).length;
